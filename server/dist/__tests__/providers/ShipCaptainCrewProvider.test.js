...
describe("ShipCaptainCrewProvider", () => {
    let provider;
    beforeEach(() => {
        provider = new ShipCaptainCrewProvider_1.ShipCaptainCrewProvider();
    });
    ...
    describe("Multiplier Calculation", () => {
        it("should return 0 multiplier when missing ship, captain, or crew", () => {
            ...
            testCases.forEach(({ hasShip, hasCaptain, hasCrew, cargoSum, expected }) => {
                const result = provider.calculateMultiplier(cargoSum, hasShip, hasCaptain, hasCrew);
        ...
        it("should return correct multipliers for different cargo sums", () => {
            ...
            testCases.forEach(({ cargoSum, expected }) => {
                const result = provider.calculateMultiplier(cargoSum, true, true, true);
                expect(result).toBe(expected);
            });
        });
    });
    describe("Dice Rolling", () => {
        it("should generate consistent dice rolls with same seeds", () => {
            const dice1 = provider.rollDice("test_seed", "client_seed", 1, 5);
            const dice2 = provider.rollDice("test_seed", "client_seed", 1, 5);
        ...
        it("should generate different dice rolls with different seeds", () => {
            const dice1 = provider.rollDice("test_seed_1", "client_seed", 1, 5);
            const dice2 = provider.rollDice("test_seed_2", "client_seed", 1, 5);
            expect(dice1).not.toEqual(dice2);
        });
        it("should generate different dice rolls with different nonces", () => {
            const dice1 = provider.rollDice("test_seed", "client_seed", 1, 5);
            const dice2 = provider.rollDice("test_seed", "client_seed", 2, 5);
            expect(dice1).not.toEqual(dice2);
        });
    });
    describe("Game Flow", () => {
        it("should process rolls in correct order (ship, captain, crew)", () => {
            ...
            provider.processRoll(mockDice, gameData);
