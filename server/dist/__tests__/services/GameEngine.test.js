...
describe("GameEngine", () => {
    ...
    describe("createGame", () => {
        it("should create a new game session successfully", async () => {
            const request = gameRequest_factory_1.createGameRequestFactory.build({
                gameType: "dice",
                betAmount: 1.0,
            ...
            const response = await GameEngine_1.default.createGame(testUser.id, request, "test_deposit_hash", clientInfo);
        ...
        it("should create games for all supported game types", async () => {
            ...
            for (const gameType of gameTypes) {
                ...
                const response = await GameEngine_1.default.createGame(testUser.id, request, "test_deposit_hash");
        ...
        it("should validate game type", async () => {
            ...
            await expect(GameEngine_1.default.createGame(testUser.id, request, "test_deposit_hash")).rejects.toThrow("Invalid game type: invalid_game");
        });
        it("should validate bet amounts", async () => {
            ...
            await expect(GameEngine_1.default.createGame(testUser.id, request, "test_deposit_hash")).rejects.toThrow("Invalid bet amount");
        });
        ...
        it("should store client information correctly", async () => {
            ...
            const session = await GameSession_1.GameSession.findOne({
        ...
        it("should generate unique session IDs", async () => {
            ...
            const response1 = await GameEngine_1.default.createGame(testUser.id, request, "test_deposit_hash");
            const response2 = await GameEngine_1.default.createGame(testUser.id, request, "test_deposit_hash");
            expect(response1.sessionId).not.toBe(response2.sessionId);
        });
        it("should use provided client seed or generate default", async () => {
            const requestWithSeed = gameRequest_factory_1.createGameRequestFactory.build({
                gameType: "dice",
                betAmount: 1.0,
                clientSeed: "custom_client_seed",
            });
            const requestWithoutSeed = gameRequest_factory_1.createGameRequestFactory.build({
                gameType: "dice",
                betAmount: 1.0,
                clientSeed: undefined,
            });
            const response1 = await GameEngine_1.default.createGame(testUser.id, requestWithSeed, "test_deposit_hash");
            const response2 = await GameEngine_1.default.createGame(testUser.id, requestWithoutSeed, "test_deposit_hash");
            const session1 = await GameSession_1.GameSession.findOne({
                sessionId: response1.sessionId,
            });
            const session2 = await GameSession_1.GameSession.findOne({
    ...
    describe("playGame", () => {
        ...
        beforeEach(async () => {
            ...
            const response = await GameEngine_1.default.createGame(testUser.id, request, "test_deposit_hash");
            gameSession = await GameSession_1.GameSession.findOne({
                sessionId: response.sessionId,
            });
        });
        it("should play a game successfully without moves", async () => {
            ...
            const response = await GameEngine_1.default.playGame(testUser.id, playRequest);
        ...
        it("should play a game with moves", async () => {
            ...
            const bjRequest = gameRequest_factory_1.createGameRequestFactory.build({
                gameType: "blackjack",
                betAmount: 1.0,
            });
            const bjResponse = await GameEngine_1.default.createGame(testUser.id, bjRequest, "test_deposit_hash");
            const bjSession = await GameSession_1.GameSession.findOne({
                sessionId: bjResponse.sessionId,
            });
            const move = gameRequest_factory_1.gameMoveFactory.build({
                action: "hit",
        ...
        it("should reject unauthorized access to game session", async () => {
            ...
            await expect(GameEngine_1.default.playGame(otherUser.id, playRequest)).rejects.toThrow("Unauthorized access to game session");
        });
        it("should reject play on non-existent session", async () => {
            const playRequest = {
                sessionId: "non_existent_session_id",
            };
            await expect(GameEngine_1.default.playGame(testUser.id, playRequest)).rejects.toThrow("Game session not found");
        });
        it("should reject play on completed session", async () => {
            ...
            await GameEngine_1.default.playGame(testUser.id, playRequest);
            // Try to play again
            await expect(GameEngine_1.default.playGame(testUser.id, playRequest)).rejects.toThrow("Game session already completed");
        });
        it("should reject play on expired session", async () => {
            // Manually expire the session
            await GameSession_1.GameSession.findByIdAndUpdate(gameSession._id, {
                expiresAt: new Date(Date.now() - 1000), // 1 second ago
            });
            const playRequest = {
                sessionId: gameSession.sessionId,
            };
            await expect(GameEngine_1.default.playGame(testUser.id, playRequest)).rejects.toThrow("Game session expired");
        });
        it("should record actions correctly", async () => {
            ...
            await GameEngine_1.default.playGame(testUser.id, playRequest);
            const updatedSession = await GameSession_1.GameSession.findById(gameSession._id);
        ...
        it("should prevent duplicate operations", async () => {
            ...
            await expect(GameEngine_1.default.playGame(testUser.id, playRequest)).rejects.toThrow("Duplicate operation detected");
        });
    ...
    describe("getGameSession", () => {
        ...
        it("should retrieve game session successfully", async () => {
            const response = await GameEngine_1.default.getGameSession(testUser.id, gameSession.sessionId);
        ...
        it("should return null for non-existent session", async () => {
            const response = await GameEngine_1.default.getGameSession(testUser.id, "non_existent_session");
        ...
        it("should reject unauthorized access", async () => {
            ...
            await expect(GameEngine_1.default.getGameSession(otherUser.id, gameSession.sessionId)).rejects.toThrow("Unauthorized access to game session");
        });
    ...
    describe("getUserGameHistory", () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < 3; i++) {
                for (const gameType of gameTypes) {
                    ...
                    // Complete some games
                    if (i % 2 === 0) {
                        await GameEngine_1.default.playGame(testUser.id, {
                            sessionId: response.sessionId,
                        });
                    }
                }
        ...
        it("should return user game history", async () => {
            const history = await GameEngine_1.default.getUserGameHistory(testUser.id);
        ...
        it("should filter by game type", async () => {
            const diceHistory = await GameEngine_1.default.getUserGameHistory(testUser.id, "dice");
        ...
        it("should respect limit parameter", async () => {
            const limitedHistory = await GameEngine_1.default.getUserGameHistory(testUser.id, undefined, 2);
        ...
        it("should return empty array for user with no games", async () => {
            ...
            const history = await GameEngine_1.default.getUserGameHistory(newUser.id);
    ...
    describe("cancelGame", () => {
        ...
        it("should cancel game successfully", async () => {
            const result = await GameEngine_1.default.cancelGame(testUser.id, gameSession.sessionId, "User requested");
            expect(result).toBe(true);
            const updatedSession = await GameSession_1.GameSession.findById(gameSession._id);
            expect(updatedSession?.status).toBe("cancelled");
        });
        it("should reject cancellation of non-existent session", async () => {
            await expect(GameEngine_1.default.cancelGame(testUser.id, "non_existent_session")).rejects.toThrow("Game session not found");
        });
        it("should reject unauthorized cancellation", async () => {
            const otherUser = await User_1.UserModel.create(user_factory_1.userFactory.build({
                walletAddress: "other_wallet",
            }));
            await expect(GameEngine_1.default.cancelGame(otherUser.id, gameSession.sessionId)).rejects.toThrow("Unauthorized access to game session");
        });
        it("should reject cancellation of completed game", async () => {
            ...
            await expect(GameEngine_1.default.cancelGame(testUser.id, gameSession.sessionId)).rejects.toThrow("Cannot cancel completed game");
        });
        it("should record cancellation reason", async () => {
            const reason = "Testing cancellation with reason";
            await GameEngine_1.default.cancelGame(testUser.id, gameSession.sessionId, reason);
            const updatedSession = await GameSession_1.GameSession.findById(gameSession._id);
            expect(updatedSession?.suspiciousActivity).toBeDefined();
            const cancellationEntry = updatedSession?.suspiciousActivity?.find((activity) => activity.reason.includes("cancelled"));
    ...
    describe("Error Handling", () => {
        it("should handle invalid user ID gracefully", async () => {
            ...
            await expect(GameEngine_1.default.createGame("invalid_user_id", request, "test_deposit_hash")).rejects.toThrow();
        });
        it("should handle database connection issues gracefully", async () => {
            ...
            try {
                await GameEngine_1.default.createGame("507f1f77bcf86cd799439011", request, "test_deposit_hash"); // Valid ObjectId format
            }
            catch (error) {
                expect(error).toBeInstanceOf(Error);
                expect(error.message).toContain("Failed to create game");
            }
        });
    });
});
